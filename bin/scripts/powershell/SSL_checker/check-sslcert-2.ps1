...
#foreach( $ipaddr in Get-Content hostnames.txt) 
#{ 

#open TCP connection
try {
	$conn = new-object system.net.sockets.tcpclient($ipaddr,443) 
	...
	try {
		...
		if ($V) {
			...
			write-host "Valid,$ipaddr,$validfrom,$validto" -ForegroundColor "Green"
	...
	write-host "SSLError,$ipaddr,na,na" -ForegroundColor "Yellow"
